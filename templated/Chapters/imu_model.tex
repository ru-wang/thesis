...
上面提到的噪声模型都是在连续时间系统下面考虑的，连续时间系统的高斯协方差（或标准差）也就称为传感器的噪声强度。实际应用中，不可能每时每刻都在读取IMU数据，而应该考虑离散时间系统的情况。\citen{smith1978exact}给出了离散时间噪声和连续时间噪声的关系。可知，离散时间系统下IMU噪声的协方差与连续时间系统存在如下关系：
\begin{equation}
\begin{aligned}
