...
\input{Chapters/alg:schur_update}
...
\input{Chapters/alg:factor_graph_aug}
...
为了保证通用性，开源的通用求解器的计算流程通常为如图~\ref{fig:framework}所示
...
\begin{table}[htb!]
\zihao{5}
\caption{增量式集束优化求解器对比}
\label{tab:dataset}
\centering
\begin{tabular}{l|cccc}
    \toprule
    求解器    & 本文提出的框架 & ICE-BA & SLAM++ & iSAM2 \\ \midrule
    线性化     &       50 &        163 &          49 &                789 \\
    线性求解    &       11 &       8366 &           0 &              20271 \\
    状态更新 &       92 &      57957 &           0 &             422163 \\
    \bottomrule
\end{tabular}
\end{table}
}
...
\begin{enumerate}
    \item 根据因子图，采用传统方法构建舒尔补并使用Cholesky分解或PCG方法求解首轮迭代；
    \item\label{it:2} 使用算法~\ref{alg:mark_dirty}标记因子图和舒尔补方程需要更新的部分；
    \item\label{it:3} 使用算法~\ref{alg:schur_update}更新舒尔补；
    \item\label{it:4} 使用Cholesky分解或PCG方法求解舒尔补方程；
    \item 重复\ref{it:2}至\ref{it:4}直到收敛；
    \item 当新增了因子或状态时，使用算法~\ref{alg:factor_graph_aug}进行增广，并重新从\ref{it:2}开始新一组优化
\end{enumerate}
